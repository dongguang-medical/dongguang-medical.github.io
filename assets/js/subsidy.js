...
(function () {
  ...
  /* 檔名用的品項簡寫：沿用門市 115 年 1 至 9 月的命名習慣
     ...
     以 +A、+B、+C 接在輪椅後面；沒有慣用寫法的（居家無障礙那一類）
     就直接用品項全名。 */
  var ABBR = {
    "便盆椅": "便",
    "沐浴椅": "沐",
  ...
  function certificateFileName(cert) {
    ...
    cert.rows.forEach(function (row) {
      ...
      if (a === undefined) a = row.itemName;
      if (a && parts.indexOf(a) === -1) parts.push(a);
    });
    /* 慣用簡寫是連寫的（沐便拐），用到全名時改空格分開才看得懂 */
    var long = parts.some(function (a) { return a.length > 4; });
    var tail = parts.join(long ? " " : "");
    var file = pad(cert.month) + pad(cert.day) + name + (tail ? "-" + tail : "");
    /* 品項全名裡可能有檔名不接受的字元；太長的也截掉，避免超過系統限制 */
    file = file.replace(/[\\/:*?"<>|]/g, "_");
    return file.slice(0, 120) + ".docx";
  }
